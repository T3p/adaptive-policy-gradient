...
class BaseExperiment(object):
    ...
    def get_trajectories_data2(self, policy, N, parallel=True):
        """Run N trajectories and returns computed data; NOW WITH SAMPLE VARIANCE
        ...
        Js = performance(rewards, self.task_prop.gamma, average=False)
        J = np.mean(Js)
        varJ = np.var(Js, ddof=1)
        gradients = self.estimator.estimate(features, actions, rewards, scores_theta, scores_sigma, trace_lengths, policy)

        return features, actions, rewards, J, varJ, gradients
...
class ExpBudget_NoDetPolicy2(BaseExperiment):
    def run(self,
            policy,
            use_local_stats=False,  # Update task prop only with local stats
            parallel=True,
        ...
        while iteration < self.constr.max_iter:
            ...
            if iteration > 1:
                self.budget += N3*(J_hat - prevJ)/N            # B += J(theta, sigma') - J(theta, sigma)
                prevJ = J_hat
                self.budget *= gamma
            ...
            self.budget += N1*(J_hat - prevJ)/N            # B += J(theta', sigma) - J(theta, sigma)
            prevJ = J_hat
...
class ExpBudget_NoDetPolicy3(BaseExperiment):
    def run(self,
            policy,
            use_local_stats=False,  # Update task prop only with local stats
            parallel=True,
        ...
        while iteration < self.constr.max_iter:
            ...
            # PERFORM FIRST STEP
            features, actions, rewards, J_hat, varJ, gradients = self.get_trajectories_data2(policy, N1, parallel=parallel)
            J_journey += J_hat * N1

            if iteration > 1:
                eps = np.sqrt(varJ / N)*sts.t.ppf(.51, N-1)
...
class ExpBudget_DetPolicy2(BaseExperiment):
    def run(self,
            policy,
            use_local_stats=False,  # Update task prop only with local stats
            parallel=True,
        ...
        while iteration < self.constr.max_iter:
            ...
            self.budget += N2*(newJ_det - prevJ_det)/N    # B += J(theta', 0) - J(theta, 0)
...
class ExpBudget_DetPolicy3(BaseExperiment):
    def run(self,
            policy,
            use_local_stats=False,  # Update task prop only with local stats
            parallel=True,
...
class Greedy_DetPolicy(BaseExperiment):
    def run(self,
            policy,
            use_local_stats=False,  # Update task prop only with local stats
            parallel=True,
        ...
        while iteration < self.constr.max_iter:
            ...
            beta, N3, safe = self.meta_selector.select_beta(policy, gradients, self.task_prop, N3, iteration, self.budget)
            policy.update_w(beta * gradients['grad_w'])
